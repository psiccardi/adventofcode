...
self.exercise1.helpers.splitCaloriesByElf = (data) => {
    return JSON.parse('[[' + data.replace(/\n/g,'|').replace(/\s/g,'').replace(/\|\|/g,'],[').replace(/\|/g, ',').trim(',') + ']]');
}

self.exercise1._executeFirst = (data) => {
    data = self.exercise1.helpers.splitCaloriesByElf(data);
    let maxElf = -1;
    let maxCal = -1;
    data.forEach((elf, i) => {
        let curCals = 0;
...
self.exercise1.executeFirst = () => {
    const [maxElf, maxCal] = self.exercise1._executeFirst(self.exercise1.data);
    return 'The elf n°' +maxElf+' carries more calories than the others. He has a total of '+maxCal;
}


self.exercise1.executeSecond = () => {
    const calories = self.exercise1._executeSecond(self.exercise1.data);
    return 'The calories carried by the three elves with more calories are: ' + calories
}